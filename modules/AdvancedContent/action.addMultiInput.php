...
if(isset($params['submit']) && $params['submit']==true)
{
	...
	if($error==false)
	{
		if(!$this->AddMultiInput($input_id,$input_fields))
		{
			$error = true;
			$errormessage .= $this->lang('error_input_id_exists').'<br />';
		}
		else if(!$this->AddTplAssoc('multi_input',$input_id,$input_tpl))
		{
			$error = true;
...
$this->smarty->assign('input_tpl_input',
		$this->CreateInputDropdown($id, 'input_tpl', $this->GetTplList('multi_input'), '', $input_tpl));
