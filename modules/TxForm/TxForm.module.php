...
class TxForm extends CMSModule
{
	...
	function __construct()
    {
		...
		require_once dirname(__FILE__).'/classes/_class.PHPMailer.php';
		...
		if (!empty($data)) {
			if (is_array($data))
			{
				...
				foreach ($data as $k=>$v){
					...
					if (is_array($v))
						foreach ($v as $k2 => $v2)
							//$data[$k1][$k2]=mysql_real_escape_string(trim(strip_tags($v2)));
							$data[$k1][$k2]=trim($v2);
					else
						//$data[$k1]=mysql_real_escape_string(trim(strip_tags($v)));
						$data[$k1]=trim($v);
					if($t) unset($data[$k]);
				}
			}
...
function CheckFormid($id,$mainid){
	$selid = '';
	$selid = $this->adb->GetOne('SELECT formid FROM cms_tx_form WHERE formid='.$id.' AND id!='.$mainid.'');
	if($selid)
		return false;
	else
		return true;
...
function formula_stogas($fields){
	
	$fields['pastato_aukstis'] = isset($fields['pastato_aukstis'])?$fields['pastato_aukstis']:0;
	$fields['stogo_apibudinimas'] = isset($fields['stogo_apibudinimas'])?$fields['stogo_apibudinimas']:1;
	$fields['sandarumas'] = isset($fields['sandarumas'])?$fields['sandarumas']:1;
	$fields['vie_tipas'] = isset($fields['vie_tipas'])?$fields['vie_tipas']:1;
	$fields['vejo_r'] = isset($fields['vejo_r'])?$fields['vejo_r']:1;
	$fields['stiprs'] = isset($fields['stiprs'])?$fields['stiprs']:0;
	
	$pastato_aukstis = $fields['pastato_aukstis'];
	...
	$past_koef = array();
	$past_koef['1'] = -0.0001*$pastato_aukstis*$pastato_aukstis+0.024*$pastato_aukstis+0.726;
	$past_koef['2'] = -0.00008*$pastato_aukstis*$pastato_aukstis+0.0196*$pastato_aukstis+0.4459;
	$past_koef['3'] = -0.00005*$pastato_aukstis*$pastato_aukstis+0.0146*$pastato_aukstis+0.2919;
	...
	$stogo_apibudinimas = $stogo_koef[$fields['stogo_apibudinimas']];
	$sandarumas = $sand_koef[$fields['sandarumas']];
	$vie_tipas = $past_koef[$fields['vie_tipas']];
	$vejo_r = $vejo_koef[$fields['vejo_r']];
	$stiprs = $fields['stiprs'];
	
	//formules
	$is_pz = -2;
	$is_kz = -3;
	$suminis_c = 1.25/2 *$vejo_r*$vejo_r *$vie_tipas*$stogo_apibudinimas-1.25/2*$vejo_r*$vejo_r*$vie_tipas*$sandarumas;
	$suminis_b = 1.25/2*$vejo_r*$vejo_r*$vie_tipas*$is_pz-1.25/2*$vejo_r*$vejo_r*$vie_tipas*$sandarumas;
	$suminis_k = 1.25/2*$vejo_r*$vejo_r*$vie_tipas*$is_kz-1.25/2*$vejo_r*$vejo_r*$vie_tipas*$sandarumas;
	
	$ret['c_zonos'] = ceil(-1*$suminis_c*1.3/$stiprs);
	$ret['b_zonos'] = ceil(-1*$suminis_b*1.3/$stiprs);
	$ret['k_zonos'] = ceil(-1*$suminis_k*1.3/$stiprs);
	return $ret;
	

}

function formula_sienos($fields){

	
	
	$fields['t_aukstis'] = isset($fields['t_aukstis'])?$fields['t_aukstis']:0;
	$fields['t_svoris'] = isset($fields['t_svoris'])?$fields['t_svoris']:1;
	$fields['vie_tipas'] = isset($fields['vie_tipas'])?$fields['vie_tipas']:1;
	$fields['vejo_r'] = isset($fields['vejo_r'])?$fields['vejo_r']:1;
	$fields['istraukimoj'] = isset($fields['istraukimoj'])?$fields['istraukimoj']:0;
	

	$t_aukstis = $fields['t_aukstis'];
	...
	*/
	...
	$past_koef = array();
	$past_koef['1'] = -0.0001*$t_aukstis*$t_aukstis+0.024*$t_aukstis+0.726;
	$past_koef['2'] = -0.00008*$t_aukstis*$t_aukstis+0.0196*$t_aukstis+0.4459;
	$past_koef['3'] = -0.00005*$t_aukstis*$t_aukstis+0.0146*$t_aukstis+0.2919;
	...
	$t_svoris = $ts_koef[$fields['t_svoris']];
	$vie_tipas = $past_koef[$fields['vie_tipas']];
	$vejo_r = $vejo_koef[$fields['vejo_r']];
	$istraukimoj = $fields['istraukimoj'];
	
	//formules
	$is_pz = -2;
	$is_kz = -3;
	$suminis_c = 1.25/2*$vejo_r*$vejo_r*$vie_tipas*0.8;
	$proj_c = $suminis_c*1.3;
	$suminis_b = -1.25/2*$vejo_r*$vejo_r*$vie_tipas*$is_pz;
	$proj_b = $suminis_b*1.3;
	$suminis_k = -1.25/2*$vejo_r*$vejo_r*$vie_tipas*$is_kz;
	$proj_k = $suminis_k*1.3;
	
	
	$ret['c_zonos'] = ceil($suminis_c*1.3/$istraukimoj*$t_svoris);
	$ret['b_zonos'] = ceil($suminis_b*1.3/$istraukimoj*$t_svoris);
	$ret['k_zonos'] = ceil($suminis_k*1.3/$istraukimoj*$t_svoris);
	return $ret;
	

}
function saveXmlCount($ip){
	$date = strtotime(date('Y-m-d'));
	//$date = strtotime(date('Y-m-d',strtotime('-7 days')));
	$info = $this->adb->GetRow("SELECT * FROM cms_xml_generation WHERE ip = '$ip'");
	if(!$info){
		$count = 1;
		$this->adb->Execute('INSERT INTO cms_xml_generation (ip,count,data) values ("'.$ip.'","'.$count.'","'.$date.'")');
		return true;
	}
	else{
		$days = round(($date - $info['data']) / (60 * 60 * 24),1);
		if($days<30 && $info['count']<1000){
			$count =$info['count']+1;
			$this->adb->Execute('UPDATE cms_xml_generation SET count="'.$count.'" WHERE id='.$info['id'].'');
			return true;
		}
		elseif($days>30){
			$count = 1;
			$this->adb->Execute('UPDATE cms_xml_generation SET count="'.$count.'",data="'.$date.'" WHERE id='.$info['id'].'');
			return true;
		}
		elseif($days<30 && $info['count']>=5)
			return false;
	}
}

function getKodas($mail){
	$kodas = $this->adb->GetOne("SELECT password FROM ".cms_db_prefix()."module_feusers_users WHERE username = '".$mail."'");
	$kodas =base64_decode($kodas);
	debug_display($kodas);	
	exit;
}
