...
{*<a href="moduleinterface.php?sp_={$smarty.get.sp_}&mact=TxForm,m1_,reimport,0">Reimport</a>*}
...
{assign var='nmb' value=$nmb+1}
				<tr class="row1" onmouseover="this.className='row1hover';" onmouseout="this.className='row1';" id='lt-{$nmb}'>
					...
					<td><div><a href="moduleinterface.php?sp_={$smarty.get.sp_}&mact=TxForm,m1_,addedit,0&prop_id={$entry->id}">{$entry->formname}</a></div></td>
					<td><div>{$entry->formalias}</div></td>
					<td>{if $entry->storedb}<div><a href="moduleinterface.php?sp_={$smarty.get.sp_}&mact=TxForm,m1_,download_export,0&prop_id={$entry->id}">Download</a></div>{/if}</td>
					<td><div><a href="moduleinterface.php?sp_={$smarty.get.sp_}&mact=TxForm,m1_,addedit,0&prop_id={$entry->id}">edit</a>&nbsp;{if $allow_del == "yes"}|&nbsp;<a onclick="if(!confirm('ar tikrai?')) return false;" href="moduleinterface.php?sp_={$smarty.get.sp_}&mact=TxForm,m1_,deleteprop,0&prop_id={$entry->id}">delete</a>{/if}</div></td>
				</tr>
				<tr style="height: 0px">
				<td colspan="6">
				</td>
