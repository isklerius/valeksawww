...
while( $dbr && ($row = $dbr->FetchRow()) )
  {
	...
	if( $value && $row['name']=='Nuotrauka')  {
	
		$fpth = $config['root_path']."/uploads/news/id$articleid/".$value;
		$furl = $config['root_url']."/uploads/news/id$articleid/".$value;
	
		$pr = '<img height="40px" src="'.$furl.'"/><br/><br/>';
	}	  
	  
	  
	$obj->field = 
	  $pr.$value.'&nbsp;'.$this->CreateFileUploadInput($id,$name).$del;;
	break;
      }
...
if( count($custom_flds) > 0 )
  {
	...
	$this->smarty->assign('wdt', $this->Lang('wdt'));
	$this->smarty->assign('hgt', $this->Lang('hgt'));
