...
//   }
  ...
  function handle_upload($itemid, $fieldname, &$error)
  {
	  ...
	  { 
	  @cms_move_uploaded_file($_FILES[$fieldname]['tmp_name'], $dest);
	   $dest2 = cms_join_path($config['uploads_path'],'news','id'.$itemid,$filename);
	   $filename3 = 'large' . $filename;
	   $filename4 = 'thumb_' . $filename;
	   $filename5 = 'thumb2_' . $filename;
	   $dest3 = cms_join_path($config['uploads_path'],'news','id'.$itemid,$filename3);
	   $dest4 = cms_join_path($config['uploads_path'],'news','id'.$itemid,$filename4);
	   $dest5 = cms_join_path($config['uploads_path'],'news','id'.$itemid,$filename5);
	  require_once(dirname(__FILE__).'/../../lib/filemanager/ImageManager/Classes/GD.php');
	  
			$img = new Image_Transform_Driver_GD;
			$img->load($dest);
			$img->resizeANDcrop(200, 150);
			$img->save($dest4);
			$img->free();
	  
			$img = new Image_Transform_Driver_GD;
			$img->load($dest);
			$img->resizeANDcrop(136, 86);
			$img->save($dest5);
			$img->free();
	 
			/*$img = new Image_Transform_Driver_GD;
			$img->load($dest);
			$img->resizeANDcrop(76, 76);
			$img->save($dest2);
			$img->free();*/
	  }
