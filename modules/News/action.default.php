...
if( $pagenumber >= $pagecount )
  {
 ...
 else
   {
    ...
    if ($params['summarytemplate']!='titlenews'){  
    ...
    for ($i=1; $i<=$pagecount; $i++){
	 $params['pagenumber'] = $i;
	 if ($i == $pagenumber) {$kas="[$i]";}else{$kas="$i";}
	 $linkai = $this->CreateFrontendLink($id,$returnid,'default', "$kas",  $params);
	 $linkai = replaceAnchorsWithText($linkai);
	 $linkas[$i][nuoroda] = $linkai;
	 $linkas[$i][numeris] = $i;
	 if ($pagenumber == $i)
		$linkas[$i][aktyvus] = 1;
	else 
 ...
 $smarty->assign_by_ref('linkai', $linkas); 
   }
