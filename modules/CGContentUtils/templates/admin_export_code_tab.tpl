...
jQuery(document).ready(function(){
  ...
  jQuery('#sel_templates').change(function(){
    var sel_templates = [];
...
<table>
 <tr>
   <td width="40%">
     ...
     <legend>{$mod->Lang('available_items')}: </legend>
       ...
       {/if}
       {if isset($modify_gcb)}
         <h4>{$mod->Lang('list_gcbs')}:</h4>
         <input type="checkbox" name="{$actionid}sel_modules" value="gcb::"/>
         <label>{$mod->Lang('list_gcbs')}</label>
         <br/>
         <br/>
       {/if}
       {if isset($modify_udt)}
