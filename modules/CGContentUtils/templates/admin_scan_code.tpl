...
function display_name_boxes()
{
   jQuery(':checkbox.import_row').each(function(){
     ...
     var sel = '[name^={/literal}{$actionid}{literal}new_name['+un+']]';
...
<table width="100" cellspacing="0" class="pagetable">
 ...
 {foreach from=$scanned_data name='scanned_data' item='row'}
  <tr>
    ...
    <td>{$row.name}</td>
    <td>
      <input type="text" name="{$actionid}new_name[{$row.uniqueid}]" value="{$row.new_name|default:''}" size="40" maxlength="40"/>
      {*$row.new_name|default:''*}
    </td>
