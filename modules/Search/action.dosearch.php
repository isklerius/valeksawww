...
class SearchItemCollection
{
  ...
  function AddItem($title, $menutext, $url, $txt, $weight = 1, $module = '', $modulerecord = 0)
  {
    if( $txt == '' ) { $txt = $url; }
    $exists = false;
		...
		if ($result->fields['module_name'] == $this->GetName())
		{
			if ($result->fields['extra_attr'] == 'content')
			{
				...
				if (isset($node))
				{
					...
					{
						$col->AddItem($content->Name(), $content->menutext(), $content->GetURL(), $content->Name(), $result->fields['total_weight']);
					}
				}
			}
