      ...
      if( $columnstodisplay['page'] )
	{
	  ...
	  if( $one->mMenuText != CMS_CONTENT_HIDDEN_NAME )
		...
		if ((check_permission($userid, 'Add Pages') && count($mypages)) || check_permission($userid,'Manage All Content'))
		{
		  ...
		  $ret = $one->whatItIs($one->mId);
		   ...
		   if (($ret[katalogai] && $ret[itemai])){
			...
			$prr = $one->Type();
			if ($prr == "sectionheader"){$prr = "content";}
		   }
		   	
		
		
			$prdadd .=  '&nbsp;&nbsp;<a href="addcontent.php'.$urlext.'&amp;parent_id='.$one->mId.'&amp;content_type='.$prr.'"> ';
	      ...
	      if( get_site_preference('listcontent_showtitle',0) )
		{
	      ...
	      if ($display == 'edit')
		$txt .= '<a href="editcontent.php'.$urlext.'&amp;content_id='.$one->mId.'&amp;page='.$page.'" title="'. cms_htmlentities($one->mName.' ('.$one->mAlias.')', '', '', true). '">'. cms_htmlentities($str, '', '', true) . '</a>'.$prdadd;
	      else
