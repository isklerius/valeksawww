 ...
 */
	...
	function toggleConstraints(kuris) 
	{
		...
		if(lock != null && lock.src != null) {
			if(lock.src.indexOf("unlocked2.gif") >= 0)
			{
				...
				jQuery('.scaleConstImg').attr({src : 'img/islocked2.gif'});
				jQuery('.constProp').attr({'checked' : 'checked'});
			...
			else
			{
				...
				jQuery('.scaleConstImg').attr({src : 'img/unlocked2.gif'});
				jQuery('.constProp').attr({'checked' : ''});
			}
