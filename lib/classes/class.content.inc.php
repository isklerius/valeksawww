...
class ContentBase
{
	...
	function whatItIs($id)
	{
		global $gCms;
		$db = $gCms->GetDb();		
		$query = 'SELECT type FROM '.cms_db_prefix().'content WHERE parent_id=? and type=? LIMIT 0,1';					
...
// 			  }
	...
	function Save()
	{
		...
		if ($this->mId == "-1"){
			$db = &$gCms->GetDb();
			$wid = $db->GenID(cms_db_prefix()."content_seq")+1;
		...
		{		
			...
			$gCms->modules[$key]['active'] == true)
			{
				...
				$direkt = $config['root_path'].'/uploads/images/pages/'.$wid.'/';
		
		if ($_POST['rm_header_image']){
			if (file_exists($direkt.'header_image.png'))
				unlink($direkt.'header_image.png');
		...
		if ($file['name']){
			...
			$fupload = $direkt.'header_image.png'; 
			
			require_once(dirname(__FILE__).'/../filemanager/ImageManager/Classes/GD.php');
